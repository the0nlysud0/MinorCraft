...
void TextureMGR::make_atlas()
{
  ...
  int atlas_width = m_Textures2D.size() * m_Textures2D[0].width;
  int atlas_height;
  ...
  for (size_t i = 0; i < m_Textures2D.size(); ++i)
{
  ...
  {
    unsigned char* src = tex.data + row * tile_size * atlas_channels;
    unsigned char* dst = atlas_data + (row * atlas_width + x_offset) * atlas_channels;
    if(src&&dst) std::memcpy(dst, src, tile_size * atlas_channels);
  }
...
{
  m_Shader.use();
  m_Shader.set_int("u_AtlasID",index);
